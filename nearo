/* FILENAME: livry-logic.js
   DESCRIPTION: Core logic for Livry Nearest Online Rider Locator
*/

(function() {
    'use strict';

    console.log("🚀 Livry External Script Loaded Successfully");
    ...
    // --- NEW: Assignment and Status Update Logic ---
    ...
    async function makeRiderOnline(event, riderId) {
        ...
        try {
            ...
            if (response.ok) {
                // Force a full recalculation and visual update
    ...
    // --- Injection Function (CONDITIONAL BUTTONS & STYLING) ---
    ...
    // --- Calculation Logic (FIXED: Get Real Status of Cleaning Rider) ---
    ...
    async function findNearestOnlineRiders(orderInfo, forceRecalculation = false) {
        ...
        let cleaningRiderStatus = 'offline'; // Default status for fixed rider

        // 1. Calculate distances for all active riders AND find the fixed rider's status
        for (const rider of ridersData.value) {

            if (rider._id === CLEANING_RIDER_ID) {
                cleaningRiderStatus = rider.status; // Capture the real status
                continue;
            }
        ...
        const fixedRider = {
             ...
             distanceKm: -1,
             status: cleaningRiderStatus // Use the captured real status
        };
